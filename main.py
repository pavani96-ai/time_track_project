...
async def log_time(employee_name: str, project: str, entry_date: str, hours: float, description: str = "") -> dict:
    ...
    return await asyncio.to_thread(db.log_time, employee_name, project, entry_date, hours, description)
...
async def get_timesheet(employee_name: str, start_date: str = "", end_date: str = "") -> list[dict]:
    ...
    return await asyncio.to_thread(db.get_timesheet, employee_name, start_date or None, end_date or None)


@mcp.tool
async def get_project_summary(project: str) -> dict:
    """Get total hours logged against a project, broken down by employee."""
    return await asyncio.to_thread(db.get_project_summary, project)
...
async def summarize_week(employee_name: str, week_start: str) -> str:
    ...
    entries = await asyncio.to_thread(db.get_timesheet, employee_name, start_date=week_start)
    if not entries:
